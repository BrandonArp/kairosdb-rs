...
mod proto {
    include!(concat!(env!("OUT_DIR"), "/io.inscopemetrics.kairosdb.proto.v2.rs"));
}
...
impl DataPointValue {
    ...
    pub fn is_numeric(&self) -> bool {
        matches!(self, DataPointValue::Long(_) | DataPointValue::Double(_) | DataPointValue::Histogram(_))
    }
...
impl HistogramData {
    ...
    pub fn from_bins(bins: Vec<(f64, u64)>, sum: f64, min: f64, max: f64, precision: Option<u8>) -> KairosResult<Self> {
        if bins.is_empty() {
        ...
        if !sum.is_finite() || !min.is_finite() || !max.is_finite() {
            return Err(KairosError::validation("Histogram sum, min, max must be finite"));
        }
        ...
        for (i, &count) in self.counts.iter().enumerate() {
            ...
            if cumulative_count >= target_count {
                if i == 0 {
                    ...
                    let bucket_start = if i > 0 { self.boundaries[i-1] } else { self.min };
    ...
    pub fn merge(&mut self, other: &HistogramData) -> KairosResult<()> {
        if self.boundaries != other.boundaries {
            return Err(KairosError::validation("Cannot merge histograms with different boundaries"));
        }
        ...
        // Recalculate mean
        let total = self.total_count();
        self.mean = if total > 0 { self.sum / total as f64 } else { 0.0 };
...
impl HistogramBuilder {
    ...
    pub fn build(self) -> KairosResult<HistogramData> {
        let bins: Vec<(f64, u64)> = self.boundaries.into_iter()
