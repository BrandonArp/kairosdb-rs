...
impl Validator {
    ...
    pub fn validate_metric_name(&self, metric: &MetricName) -> KairosResult<()> {
        ...
        if metric.as_str().contains("..") {
            return Err(KairosError::validation(
                "Metric name cannot contain consecutive dots",
            ));
        }
        ...
        if metric.as_str().starts_with('.') || metric.as_str().ends_with('.') {
            return Err(KairosError::validation(
                "Metric name cannot start or end with dots",
            ));
        }
    ...
    pub fn validate_tag_key(&self, key: &TagKey) -> KairosResult<()> {
        ...
        if key.as_str().starts_with("kairosdb_") {
            return Err(KairosError::validation(
                "Tag key cannot start with 'kairosdb_' (reserved)",
            ));
        }
    ...
    pub fn validate_tag_value(&self, value: &TagValue) -> KairosResult<()> {
        ...
        if value.as_str().chars().any(|c| c.is_control() && c != '\t') {
            return Err(KairosError::validation(
                "Tag value contains invalid control characters",
            ));
        }
        ...
        match value {
            ...
            DataPointValue::Complex { real, imaginary } => {
                if real.is_infinite() || real.is_nan() {
                    return Err(KairosError::validation(
                        "Complex real part cannot be infinite or NaN",
                    ));
                }
                if imaginary.is_infinite() || imaginary.is_nan() {
                    return Err(KairosError::validation(
                        "Complex imaginary part cannot be infinite or NaN",
                    ));
                }
            }
            DataPointValue::Text(text) => {
                ...
                {
                    return Err(KairosError::validation(
                        "Text value contains invalid control characters",
                    ));
                }
            }
            ...
            DataPointValue::Histogram(hist) => {
                ...
                if hist.boundaries.is_empty() {
                    return Err(KairosError::validation(
                        "Histogram must have at least one boundary",
                    ));
                }
                ...
                if counts_sum != hist.total_count() {
                    return Err(KairosError::validation(
                        "Histogram counts sum must equal total_count",
                    ));
                }
                ...
                for boundary in &hist.boundaries {
                    if boundary.is_infinite() || boundary.is_nan() {
                        return Err(KairosError::validation(
                            "Histogram boundary cannot be infinite or NaN",
                        ));
                    }
                }
                ...
                if hist.sum.is_infinite() || hist.sum.is_nan() {
                    return Err(KairosError::validation(
                        "Histogram sum cannot be infinite or NaN",
                    ));
                }

                if hist.min.is_infinite() || hist.min.is_nan() {
                    return Err(KairosError::validation(
                        "Histogram min cannot be infinite or NaN",
                    ));
                }

                if hist.max.is_infinite() || hist.max.is_nan() {
                    return Err(KairosError::validation(
                        "Histogram max cannot be infinite or NaN",
                    ));
                }
            }
        }
    ...
    pub fn validate_data_point_batch(&self, batch: &DataPointBatch) -> KairosResult<()> {
        ...
        for i in 0..batch.points.len() {
            for j in i + 1..batch.points.len() {
                ...
                if p1.metric == p2.metric && p1.timestamp == p2.timestamp && p1.tags == p2.tags {
                    return Err(KairosError::validation(
                        "Batch contains duplicate data points",
                    ));
                }
            }
        }
    ...
    pub fn validate_time_range(&self, range: &TimeRange) -> KairosResult<()> {
        ...
        if duration > self.limits.max_query_range_ms {
            return Err(KairosError::validation(format!(
                ...
                duration, self.limits.max_query_range_ms
            )));
        }
    ...
    pub fn validate_query_request(&self, query: &QueryRequest) -> KairosResult<()> {
        ...
        if query.metrics.is_empty() {
            return Err(KairosError::validation(
                "Query must specify at least one metric",
            ));
        }
    ...
    pub fn validate_metric_query(&self, query: &MetricQuery) -> KairosResult<()> {
        ...
        for (key, value) in &query.tags {
            if key.is_empty() || value.is_empty() {
                return Err(KairosError::validation(
                    "Tag filter key and value cannot be empty",
                ));
            }
        }
...
mod tests {
    ...
    fn test_metric_name_validation() {
        ...
        // Valid metric names
        assert!(validator
            .validate_metric_name(&MetricName::new("simple").unwrap())
            .is_ok());
        assert!(validator
            .validate_metric_name(&MetricName::new("with.dots").unwrap())
            .is_ok());
        assert!(validator
            .validate_metric_name(&MetricName::new("with_underscores").unwrap())
            .is_ok());
        assert!(validator
            .validate_metric_name(&MetricName::new("with-dashes").unwrap())
            .is_ok());
    ...
    fn test_batch_validation() {
        ...
        batch
            .add_point(DataPoint::new_long("metric1", Timestamp::now(), 1))
            .unwrap();
        batch
            .add_point(DataPoint::new_long("metric2", Timestamp::now(), 2))
            .unwrap();
