...
fn test_row_key_binary_format() {
    ...
    tags.insert(TagKey::new("application").unwrap(), TagValue::new("database").unwrap());
    tags.insert(TagKey::new("datacenter").unwrap(), TagValue::new("us-west-1").unwrap());
    tags.insert(TagKey::new("host").unwrap(), TagValue::new("server-01").unwrap());
...
fn test_column_key_format() {
    ...
    let column_int = u32::from_be_bytes([
        column_bytes[0], column_bytes[1], column_bytes[2], column_bytes[3]
    ]);
    
    // Should represent (timestamp - row_time) << 1
    let expected_offset = ((timestamp.timestamp_millis() - row_time.timestamp_millis()) << 1) as u32;
    assert_eq!(column_int, expected_offset);
    
    println!("Column key: 0x{:08x} (offset: {})", column_int, expected_offset >> 1);
}
...
fn test_tag_format_compatibility() {
    ...
    for (i, tag_pairs) in test_cases.iter().enumerate() {
        ...
        if tag_pairs.is_empty() {
            ...
            assert!(cassandra_format.contains('='), "Test case {}: {}", i, cassandra_format);
            // Should end with colon
            assert!(cassandra_format.ends_with(':'), "Test case {}: {}", i, cassandra_format);
            // Should NOT use old := format
            assert!(!cassandra_format.contains(":="), "Test case {}: {}", i, cassandra_format);
            
            // Tags should be sorted alphabetically
            if tag_pairs.len() > 1 {
                let tag_parts: Vec<&str> = cassandra_format.split(':').filter(|s| !s.is_empty()).collect();
                let mut sorted_pairs = tag_pairs.clone();
                sorted_pairs.sort_by(|a, b| a.0.cmp(b.0));
                
                for (j, (key, value)) in sorted_pairs.iter().enumerate() {
                    if j < tag_parts.len() {
                        assert!(tag_parts[j].starts_with(&format!("{}={}", key, value)), 
                               "Test case {}, tag {}: expected '{}={}' in position {}, got '{}'", 
                               i, j, key, value, j, tag_parts[j]);
                    }
                }
            }
        }
...
fn test_value_serialization_format() {
    let test_cases = vec![
        ...
        ("binary_data", DataPointValue::Binary(vec![0x00, 0xFF, 0x42])),
    ];
    ...
    for (name, value) in test_cases {
        ...
        match &value {
            DataPointValue::Double(_) => {
                ...
                assert_eq!(cassandra_value.bytes.len(), 8, "Double {} should be 8 bytes", name);
            },
            DataPointValue::Long(v) => {
                ...
                if *v == 0 {
                    ...
                    assert!(cassandra_value.bytes.len() <= 8, "Long {} should be <= 8 bytes", name);
                    assert!(cassandra_value.bytes.len() > 0, "Long {} should be > 0 bytes", name);
                    // Should not have leading zeros (except for the value 0)
                    assert_ne!(cassandra_value.bytes[0], 0, "Long {} should not have leading zero", name);
                }
            },
            DataPointValue::Text(s) => {
                assert_eq!(cassandra_value.bytes.len(), s.as_bytes().len(), "Text {} length mismatch", name);
                assert_eq!(cassandra_value.bytes, s.as_bytes(), "Text {} content mismatch", name);
            },
            DataPointValue::Binary(b) => {
                assert_eq!(cassandra_value.bytes, *b, "Binary {} content mismatch", name);
            },
            _ => {}
        }
...
fn test_row_time_bucketing() {
    ...
    for ts_millis in test_timestamps {
        ...
        assert_eq!(row_time.timestamp_millis() % three_weeks_ms, 0, 
                  "Row time {} is not on 3-week boundary", row_time.timestamp_millis());
        
        // Row time should be <= original timestamp
        assert!(row_time.timestamp_millis() <= ts_millis, 
               "Row time {} should be <= timestamp {}", row_time.timestamp_millis(), ts_millis);
        
        println!("Timestamp {} -> Row time {}", ts_millis, row_time.timestamp_millis());
        
        // Check monotonicity (row times should be non-decreasing)
        if let Some(prev) = previous_row_time {
            assert!(row_time.timestamp_millis() >= prev, 
                   "Row time should be non-decreasing: {} >= {}", row_time.timestamp_millis(), prev);
        }
...
fn test_data_type_assignment() {
    ...
    for (value, expected_type) in test_cases {
        let mut data_point = match &value {
            DataPointValue::Double(d) => DataPoint::new_double("test", Timestamp::now(), d.into_inner()),
        ...
        assert_eq!(actual_type, expected_type, "Data type mismatch for {:?}", value);
...
fn test_round_trip_value_serialization() {
    ...
    for original_value in test_values {
        ...
        assert!(round_trip_result.is_ok(), "Failed to deserialize {:?}: {:?}", 
               original_value, round_trip_result.err());
        ...
        match (&original_value, &round_trip_value) {
            (DataPointValue::Double(orig), DataPointValue::Double(rt)) => {
                assert!((orig.into_inner() - rt.into_inner()).abs() < f64::EPSILON, 
                       "Double round-trip failed: {} != {}", orig.into_inner(), rt.into_inner());
            },
            ...
            _ => panic!("Type mismatch in round-trip: {:?} vs {:?}", original_value, round_trip_value),
        }
