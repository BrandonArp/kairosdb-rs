...
impl PerfTestRunner {
    pub fn new(config: PerfTestConfig) -> Self {
        ...
        if config.concurrent_batches > 1000 {
            tracing::warn!("Very high concurrency setting ({}), this may overwhelm the system", config.concurrent_batches);
        }
    ...
    async fn run_main_test(&self) -> anyhow::Result<PerfTestResults> {
        ...
        let results_state = Arc::new(tokio::sync::Mutex::new((0u64, 0u64, 0u64, Vec::<String>::new()))); // (successful, failed, total_datapoints, errors)
        ...
        for worker_id in 0..self.config.concurrent_batches {
            ...
            let handle = tokio::spawn(async move {
                Self::consumer_worker(worker_id, receiver, client, ingest_url, performance_mode, latencies_clone, results_clone).await;
            });
        ...
        for producer_id in 0..self.config.producer_tasks {
            ...
            let handle = tokio::spawn(async move {
                let generator = crate::performance::generator::MetricDataGenerator::new(generator_config);
                Self::producer_worker(producer_id, generator, producer_batch_size, test_duration, sender).await
            });
        ...
        info!("All producers completed: generated {} total batches", total_batches);
        ...
        info!("Test completed: {}s test duration, {}s total elapsed", 
              test_duration.as_secs(), total_elapsed.as_secs());
            ...
            match batch_sender.send((batch, datapoint_count)).await {
                Ok(_) => {
                    ...
                    if last_progress.elapsed() >= Duration::from_secs(10) {
                        ...
                        info!("Producer progress: {}s / {}s ({:.1}%) - {} batches generated", 
                              elapsed, total, (elapsed as f64 / total as f64) * 100.0, batches_generated);
            ...
            match batch_sender.send((batch, datapoint_count)).await {
                Ok(_) => {
                    ...
                    if last_progress.elapsed() >= Duration::from_secs(30) {
                        ...
                        let total = test_duration.as_secs();
                        info!("Producer {} progress: {}s / {}s ({:.1}%) - {} batches generated", 
                              producer_id, elapsed, total, (elapsed as f64 / total as f64) * 100.0, batches_generated);
                        last_progress = Instant::now();
                    }
                }
                Err(_) => {
    ...
    async fn consumer_worker(
        worker_id: usize,
        batch_receiver: Arc<tokio::sync::Mutex<mpsc::Receiver<(Vec<Vec<Value>>, u64)>>>,
        client: Client,
        ingest_url: String,
        performance_mode: Option<String>,
        latencies: Arc<tokio::sync::Mutex<Vec<f64>>>,
        results_state: Arc<tokio::sync::Mutex<(u64, u64, u64, Vec<String>)>>, // (successful, failed, total_datapoints, errors)
    ) {
        ...
        loop {
            ...
            match message {
                Some((batch, datapoint_count)) => {
                    ...
                    match Self::send_batch_internal(&client, &ingest_url, &batch, performance_mode.as_deref()).await {
                        Ok(_) => {
        ...
        info!("Worker {} finished: processed {} requests", worker_id, requests_processed);
    }

    async fn send_batch(&self, batch: &[Vec<Value>]) -> anyhow::Result<()> {
        Self::send_batch_internal(&self.client, &self.config.ingest_url, batch, self.config.performance_mode.as_deref()).await
    }
