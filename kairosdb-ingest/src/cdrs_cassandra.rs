//! Proper CDRS-tokio Cassandra client implementation
//!
//! This module provides a real Cassandra client using CDRS-tokio for actual data writes
//! to Cassandra in KairosDB format.
...
// CDRS-tokio imports
use cdrs_tokio::{
    authenticators::SaslAuthenticatorProvider,
    cluster::{TcpConnectionManager, session::Session, KeyspaceHolder},
...
/// Type alias for the session we'll use
...
/// Statistics for the CDRS Cassandra client
#[derive(Debug, Clone, Default)]
pub struct CdrsStats {
    pub total_queries: u64,
    pub failed_queries: u64,
    pub avg_query_time_ms: u64,
}

/// CDRS-tokio Cassandra client for real data writes
pub struct CdrsCassandraClient {
...
impl CdrsCassandraClient {
    /// Create a new CDRS Cassandra client
    pub async fn new(config: Arc<CassandraConfig>) -> Result<Self> {
        info!("Creating CDRS Cassandra client...");
        ...
        // Create keyspace holder
        let (keyspace_tx, keyspace_rx) = tokio::sync::watch::channel(None);
        let keyspace_holder = Arc::new(KeyspaceHolder::new(keyspace_tx));
        
        // Create connection manager - let's use what the API actually expects
        let manager = TcpConnectionManager::new(
            (host, port),
            keyspace_holder.clone(),
            Box::new(cdrs_tokio::frame_encoding::ProtocolFrameEncodingFactory),
            Compression::None,
            4, // pool size
            true, // tcp_nodelay
            Version::V4,
        ...
        // Create session - try direct instantiation with corrected argument order
        let session = Session::new(
            ...
            Version::V4,
            false, // beta_protocol
        ).await.context("Failed to create Cassandra session")?;
    ...
    async fn ensure_keyspace_and_tables(&self) -> Result<()> {
        ...
        // Create KairosDB row_keys table
        ...
        // Create KairosDB string_index table
    ...
    /// Get statistics
    pub fn get_stats(&self) -> CdrsStats {
        CdrsStats {
