...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "  Batches/sec: {:.2}",
        num_batches as f64 / enqueue_duration.as_secs_f64()
    );
    info!(
        "  Points/sec: {:.2}",
        total_points as f64 / enqueue_duration.as_secs_f64()
    );
    info!("  Queue size: {}", queue_size_after_enqueue);
    if num_batches > 0 {
        info!(
            "  Avg enqueue time: {:?}",
            enqueue_duration / num_batches as u32
        );
        enqueue_times.sort();
        ...
        match queue.claim_work_items_batch(1000, 1000) {
            Ok(work_items) if !work_items.is_empty() => {
                ...
                for work_item in &work_items {
                    ...
                    if let Err(e) = queue.remove_processed_item(
                        &work_item.queue_key,
                        work_item.entry.batch.points.len(),
                    ) {
    ...
    info!(
        "  Batches/sec: {:.2}",
        dequeued_batches as f64 / dequeue_duration.as_secs_f64()
    );
    info!(
        "  Points/sec: {:.2}",
        dequeued_points as f64 / dequeue_duration.as_secs_f64()
    );
    info!("  Queue size: {}", queue_size_after_dequeue);
    if dequeued_batches > 0 {
        info!(
            "  Avg dequeue time: {:?}",
            dequeue_duration / dequeued_batches as u32
        );
        dequeue_times.sort();
        info!("  Dequeue p50: {:?}", dequeue_times[dequeued_batches / 2]);
        info!(
            "  Dequeue p99: {:?}",
            dequeue_times[dequeued_batches * 99 / 100]
        );
    ...
    let consumer_handle = tokio::spawn(async move {
            ...
            match queue_clone2.claim_work_items_batch(1000, 1000) {
                Ok(work_items) if !work_items.is_empty() => {
                    ...
                    for work_item in &work_items {
                        total_points += work_item.entry.batch.points.len();
                        // Remove processed items
                        if let Err(e) = queue_clone2.remove_processed_item(
                            &work_item.queue_key,
                            work_item.entry.batch.points.len(),
                        ) {
                            eprintln!("Failed to remove processed item: {}", e);
        ...
        info!(
            "Consumer finished: {} batches, {} points",
    ...
    info!(
        "  Disk usage: {:.2} MB",
        metrics.disk_usage_bytes.get() / 1_048_576.0
    );
    ...
    if let Ok(disk_usage) = queue.get_disk_usage_bytes() {
        ...
        info!(
            "  Bytes per point: {:.2}",
            disk_usage as f64 / total_points as f64
        );
    }
...
fn generate_test_batches(
    num_batches: usize,
    points_per_batch: usize,
) -> Result<Vec<DataPointBatch>, Box<dyn std::error::Error>> {
    ...
    for batch_idx in 0..num_batches {
        ...
        for point_idx in 0..points_per_batch {
            ...
            tags.insert(
                "host".to_string().into(),
                format!("server{}", batch_idx % 10).into(),
            )?;
            tags.insert(
                "region".to_string().into(),
                format!("region{}", batch_idx % 3).into(),
            )?;
            tags.insert(
                "service".to_string().into(),
                "test-service".to_string().into(),
            )?;

            let data_point = DataPoint {
                metric: MetricName::from(format!("test.metric.{}", point_idx % 5)),
                timestamp: Timestamp::from_millis(
                    base_timestamp + (batch_idx * points_per_batch + point_idx) as i64 * 1000,
                )?,
