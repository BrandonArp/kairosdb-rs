...
impl AggregationEngine {
    ...
    fn apply_single_aggregation(
        ...
        match aggregator {
            ...
            Aggregator::Percentile { percentile, sampling } => {
    ...
    fn apply_rate(
        ...
        for i in 1..data_points.len() {
            ...
            if let (Some(prev_val), Some(curr_val)) = (prev.value.as_f64(), curr.value.as_f64()) {
                let time_diff_ms = curr.timestamp.timestamp_millis() - prev.timestamp.timestamp_millis();
                if time_diff_ms > 0 {
    ...
    fn apply_sampler(
        ...
        debug!("Sampler aggregation with unit '{}' - returning original data", unit);
        Ok(data_points)
    }
    ...
    fn group_by_sampling(
        ...
        if let Some(sampling) = sampling {
            ...
            for point in data_points {
                ...
                groups.entry(rounded_ts).or_insert_with(Vec::new).push(point);
            }
        } else {
    ...
    fn calculate_sum(&self, points: &[DataPoint]) -> Option<DataPointValue> {
        ...
        if has_float {
            Some(DataPointValue::Double(OrderedFloat(float_sum + int_sum as f64)))
        } else if has_int {
    ...
    fn calculate_percentile(&self, points: &[DataPoint], percentile: f64) -> Option<DataPointValue> {
        let mut numeric_values: Vec<f64> = points
            .iter()
            .filter_map(|p| p.value.as_f64())
            .collect();
    ...
    fn get_sampling_interval_ms(&self, sampling: &kairosdb_core::query::Sampling) -> KairosResult<i64> {
        let multiplier = match sampling.unit.as_str() {
            ...
            "hours" => 60 * 60 * 1000,
            "days" => 24 * 60 * 60 * 1000,
            _ => return Err(KairosError::validation(format!("Unknown sampling unit: {}", sampling.unit))),
        };
    ...
    fn get_time_unit_multiplier(&self, unit: &str) -> KairosResult<f64> {
        match unit {
            ...
            _ => Err(KairosError::validation(format!("Unknown time unit: {}", unit))),
        }
    }
    
    /// Round timestamp to sampling interval
    fn round_timestamp_to_interval(&self, timestamp: Timestamp, interval_ms: i64) -> KairosResult<Timestamp> {
