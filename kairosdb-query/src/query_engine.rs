...
use kairosdb_core::{
    ...
    query::{
        QueryRequest, QueryResponse, MetricQueryResult, QueryResult, DataPointResult,
        MetricNamesQuery, MetricNamesResponse,
        TagNamesQuery, TagNamesResponse,
        TagValuesQuery, TagValuesResponse,
    },
    time::{TimeRange, Timestamp},
    cassandra::{RowKey, ColumnName, CassandraValue, TableNames},
...
use crate::{config::QueryConfig, aggregation::AggregationEngine};
...
impl QueryEngine {
    ...
    pub async fn new(config: Arc<QueryConfig>) -> Result<Self> {
        ...
        info!("Query engine initialized with cache: {}", config.cache.enable_caching);
    ...
    pub async fn execute_query(&self, request: QueryRequest) -> KairosResult<QueryResponse> {
        ...
        for metric_query in &request.metrics {
            match self.execute_metric_query(metric_query, &time_range, request.limit).await {
                Ok(result) => {
                    total_sample_size += result.sample_size;
                    query_results.push(result);
                }
                Err(err) => {
                    error!("Failed to execute metric query for '{}': {}", 
                           metric_query.name.as_str(), err);
                    return Err(err);
                }
            }
        }
        ...
        self.record_query_completion(query_time, total_sample_size).await;
        
        info!("Query completed in {:?}, returned {} samples", query_time, total_sample_size);
    ...
    async fn execute_metric_query(
        ...
        let time_series = self.find_time_series(&metric_query.name, &metric_query.tags, time_range).await?;
        ...
        for series_info in time_series {
            ...
            let data_points = self.query_time_series_data(&series_info, time_range).await?;
            
            // Apply aggregations if specified
            let processed_points = if metric_query.aggregators.is_empty() {
                data_points
            } else {
                self.aggregation_engine.apply_aggregations(&data_points, &metric_query.aggregators)?
            };
            
            // Apply limits
            let limited_points = self.apply_limits(processed_points, metric_query.limit, global_limit);
    ...
    async fn query_time_series_data(
        ...
        debug!("Generated {} data points for time series", data_points.len());
        Ok(data_points)
    }
    ...
    /// Get metric names
    pub async fn get_metric_names(&self, query: MetricNamesQuery) -> KairosResult<MetricNamesResponse> {
    ...
    pub async fn get_tag_values(&self, query: TagValuesQuery) -> KairosResult<TagValuesResponse> {
        ...
        let mut results = match query.tag_name.as_str() {
            "host" => vec!["server1".to_string(), "server2".to_string(), "server3".to_string()],
            "region" => vec!["us-east-1".to_string(), "us-west-2".to_string(), "eu-west-1".to_string()],
            "environment" => vec!["production".to_string(), "staging".to_string(), "development".to_string()],
            _ => vec![],
        };
    ...
    async fn record_query_completion(&self, duration: Duration, sample_size: usize) {
        ...
        metrics.avg_query_time_ms = metrics.total_query_time_ms as f64 / metrics.queries_executed as f64;
    }
...
mod tests {
    ...
    async fn test_metric_names_query() {
        ...
        assert!(response.results.iter().all(|name| name.starts_with("system")));
    }
}